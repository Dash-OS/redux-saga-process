 ...
 * @param  {Object}    monitorConfig  [description]
 * @param  {[type]}    compiledConfig [description]
 * @return {Generator}                [description]
 */
export default function* subscribeToTypes(proc, monitorConfig) {
  const { schema } = proc;
  ...
  */
  if (schema.startOnAction && typeof schema.instance.processStarts === 'function') {
  ...
  if (monitorPattern !== DO_NOT_MONITOR || cancelPattern !== DO_NOT_MONITOR) {
    return yield fork([this, monitorTypesSaga], monitorPattern, cancelPattern, proc, {
